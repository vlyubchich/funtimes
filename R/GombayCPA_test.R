...
GombayCPA_test <- function(y, a.order, alternatives = c("two-sided", "greater", "lesser", "temporary"), crit.type = c("asymptotic", "bootstrap"), num.bootstrap = 1000) {
    ...
    if (crit.type == "bootstrap") {
        ...
        k_seq <- 1:100  # 100 terms is more than enough for convergence up to 16 decimal places
        k_sq  <- k_seq^2
        ...
        for (i in 1:(a.order + 2)) {
            ...
            if (alternatives == "greater" || alternatives == "lesser") {
                ...
                # Splitting negative and positive k is mathematically equivalent to
                # 2 * sum(positive k). This eliminates the 2-million element vector memory drain.
                eqnb_c <- sum(((-1)^(k_seq + 1)) * exp(-2 * SQtest_stat * k_sq))
                p_tmp  <- 2 * eqnb_c
                asympvals[i] <- min(max(p_tmp, 0), 1) # Force boundary bounds [0, 1]

            } else if (alternatives == "temporary") {
                eqnd <- 1 - sum(2 * ((4 * k_sq * SQtest_stat) - 1) * exp(-2 * k_sq * SQtest_stat))
                asympvals[i] <- min(max(eqnd, 0), 1)
            }
        }
