Clustering <- function(formula, rate = 1, alpha = 0.05, ...){
    require(funtimes) 
    # The matrix should have each column with equal number of rows. 
    ######################### Clustering #######################################

    # Storing the final list of clusters 
    Lfinal <- list() 
    clus_col.Idx <- list() # Sroring the index of columns in cluster
